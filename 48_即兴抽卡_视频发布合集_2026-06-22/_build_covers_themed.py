# -*- coding: utf-8 -*-
"""
即兴抽卡 · 第 07/08/09 组封面 (2026-06-23 起) —— 主题与前六组不同, 封标按题材各自定制
依据 KB: 04_方法论/05_prompt与工具方法/代码排版小红书封面工作流
        + skill aigc-poster-layout(封标设计语言随题材切换) + 网络复盘

07 假面 (红衣戏面) —— 暗调戏剧:  宋体竖排骨白主标 + 朱红细竖线 + 朱砂印章 + 角落柔暗(护脸/护面具)
08 栖白 (白西装编辑感) —— 时尚编辑: Playfair Didone 高反差报头(顶部) + 微软雅黑封面语(精简两行)
09 霜瞳 (银发蓝瞳cosplay) —— 高调薄雾·冷调: 冷墨蓝宋体竖排 + 冰蓝细线 + 白色柔光晕托字(深字浅底)
共用一套 cover-fit / 柔投影 / 2x 超采样 脚手架, 文本层各走各的设计; 封面均不挂作者/栏目/期号。
"""
import os, glob
import numpy as np
...
# ---- 09 霜瞳 调色(高调薄雾/冷调) ----
ICE   = (36, 49, 66)      # 冷墨蓝(主标) — 高调留白上用深字才读得清
...
# ============ 设计 C: 09 霜瞳 — 高调薄雾·冷调空灵(cosplay) ============
def design_ether(base, w, h, W, H, cfg, portrait):
    yy, xx = np.mgrid[0:h, 0:w]
    # 左上角极轻冷调压暗, 只为把深色标题从薄雾里托出来(不破高调)
    cx, cy = 0.08 * w, 0.06 * h
    rad = np.clip(1 - (((xx - cx) / (0.46 * w)) ** 2 + ((yy - cy) / (0.5 * h)) ** 2), 0, 1) ** 1.6
    base = Image.alpha_composite(base, scrim_layer(w, h, rad * 60, (44, 60, 82)))
    ...
    # 文本块靠左上: 主标在左, 冰蓝细线, 副标在右(向画面内侧)
    mgx = int(w * (0.085 if portrait else 0.05))
    title_x = mgx + ts // 2
    line_x = title_x + ts // 2 + int(H * 0.020) * SS
    sub_x = line_x + int(H * 0.020) * SS + ss_ // 2

    block_h = len(title) * t_step
    cy0 = int(h * (0.31 if portrait else 0.40))
    t_top = cy0 - block_h // 2 + t_step // 2
    vtext(d, title_x, t_top, title, f_song(ts), ICE, t_step)

    line_top = t_top - t_step // 2 + int(ts * 0.10)
    line_bot = t_top + block_h - t_step // 2 + int(ts * 0.10)
    d.line([(line_x, line_top), (line_x, line_bot)], fill=GLAZE + (235,), width=max(2, SS + 1))

    vtext(d, sub_x, line_top + s_step // 2 + int(H * 0.012) * SS, sub, f_kai(ss_), ICE2, s_step)

    # 拉丁落款(左下, 冰蓝) + 冰蓝短线
    foot_y = int(h * 0.945)
    ls_ = int(H * 0.024) * SS
    d.line([(mgx, foot_y), (mgx + int(H * 0.068) * SS, foot_y)], fill=GLAZE + (220,), width=max(2, SS))
    tracked(d, mgx, foot_y + int(H * 0.010) * SS, cfg["lat"], f_deng(ls_), GLAZE + (240,),
            int(H * 0.006) * SS)

    out = base.copy()
    # 深字浅底: 先垫一层白色柔光晕(分离), 再压极淡冷投影
    halo, _ = soft_shadow(txt, w, h, rgb=HALO, k=0.95, blur=7, dy=0)
    out.alpha_composite(halo)
    shadow, dy = soft_shadow(txt, w, h, rgb=(20, 30, 44), k=0.30, blur=4, dy=2)
    out.alpha_composite(shadow, (0, dy * SS))
    out.alpha_composite(txt)
    return out
