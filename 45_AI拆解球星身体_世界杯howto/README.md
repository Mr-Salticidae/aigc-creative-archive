...
## 出图前自检 checklist（每期套用）
> 经验来源：第02期图2标题"≠"前误连逗号、连锁反应句全角逗号混用——小字标点肉眼易漏，**审脚本里的文案源串比看图更可靠**（`grep` 文案串过一遍）。

**标点**
- [ ] 全/半角统一（全篇逗号、冒号、问号风格一致，不混用）
- [ ] 符号前后无误连标点（`≠`/`→`/`｜` 前后不该有逗号）
- [ ] `≠`、`｜` 等符号两侧空格一致（如 `醒来 ≠ 失眠`）
- [ ] 无渲染豆腐块（雅黑无彩色 emoji，图内禁用 emoji；emoji 只放发布文案）

**文字**
- [ ] 标题/正文无错别字、无多余空格
- [ ] 数字单位统一（`16~18℃`、`90 分钟` 等格式一致）

**版式**
- [ ] 文字不出血、不压到看不清的纹理上；行末不孤字
- [ ] 胶囊标签期数正确（`· 0X`）；配色用栏目色
- [ ] 封面：标题与主体对角平衡、不偏沉（见上"构图经验"）
